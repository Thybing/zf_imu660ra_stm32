...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
static imu963ra_state_enum imu963ra_write_mag_register (imu_interface_obj_struct *interface, uint8_t addr, uint8_t reg, uint8_t data)
{
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
static imu963ra_state_enum imu963ra_read_mag_register (imu_interface_obj_struct *interface, uint8_t addr, uint8_t reg, uint8_t *data)
{
    imu963ra_state_enum return_state = IMU963RA_STATE_NO_ERROR;
    uint16_t              timeout_count = 0;
    ...
    do{
        ...
        interface->read_8bit_registers(
            IMU963RA_DEV_ADDR, 
                ...
                :   IMU963RA_OUTX_L_A,
            data, 6);
        measurement_data->x = (int16_t)(((uint16_t)data[1] << 8 | data[0]));
        measurement_data->y = (int16_t)(((uint16_t)data[3] << 8 | data[2]));
        measurement_data->z = (int16_t)(((uint16_t)data[5] << 8 | data[4]));
    }while(0);
    return return_state;
}
        ...
        switch((item >> IMU963RA_CONFIG_TYPE_OFFSET) & IMU963RA_CONFIG_TYPE_MASK)
        {
            ...
            case 0x01:
            {
                ...
                IMU963RA_DEV_ADDR, 
                    ...
                    ? (IMU963RA_CTRL1_XL | IMU963RA_SPI_R)
                    : IMU963RA_CTRL1_XL);
                const uint8_t acc_range_list[6] = {0x00, 0x00, 0x00, 0x08, 0x0C, 0x04};
            ...
            case 0x03:
            {
                ...
                IMU963RA_DEV_ADDR, 
                    ...
                    ? (IMU963RA_CTRL2_G | IMU963RA_SPI_R)
                    : IMU963RA_CTRL2_G);
                const uint8_t gyro_range_list[8] = {0x00, 0x00, 0x02, 0x00, 0x04, 0x08, 0x0C, 0x01};
